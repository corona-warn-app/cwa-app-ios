...
class KeySubmissionResourceTests: XCTestCase {
	...
	func testSubmit_Response400() {
		...
		restServiceProvider.load(resource) { result in
			...
			case let .failure(error):
				guard case ServiceError.receivedResourceError(.invalidPayloadOrHeaders) = error else {
	...
	func testSubmit_Response403() {
		...
		restServiceProvider.load(resource) { result in
			...
			case .success:
				XCTFail("error expected")
			case let .failure(error):
				guard case ServiceError.receivedResourceError(.invalidTan) = error else {
					XCTFail("We expect error to be of type invalidPayloadOrHeaders")
					return
				}
			}
