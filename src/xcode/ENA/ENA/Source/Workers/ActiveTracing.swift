...
struct ActiveTracing: Equatable {
	...
	var inHours: Int {
		...
		return 24 * 14 //Int((interval / TimeInterval.SEC_PER_HOUR).rounded(.down))
	}
	
	var inDays: Int {
		return 14 // min(Int((interval / TimeInterval.SEC_PER_DAY).rounded(.toNearestOrAwayFromZero)), maximumNumberOfDays)
	}
}
