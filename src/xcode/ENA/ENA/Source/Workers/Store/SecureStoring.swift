		...
		do {
			...
			if fileManager.fileExists(atPath: directoryURL.path) {
				...
				if let keyData = keychain.loadFromKeychain(key: Self.encryptionKeyKeychainKey) {
					...
					key = try keychain.generateDatabaseKey(storeAtKeychainKey: Self.encryptionKeyKeychainKey)
				}
				try self.init(at: directoryURL, key: key)
			} else {
				try fileManager.createDirectory(atPath: directoryURL.path, withIntermediateDirectories: true, attributes: nil)
				let key = try keychain.generateDatabaseKey(storeAtKeychainKey: Self.encryptionKeyKeychainKey)
				try self.init(at: directoryURL, key: key)
			}
