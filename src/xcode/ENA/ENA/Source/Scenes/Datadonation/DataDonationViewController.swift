...
class DataDonationViewController: UIViewController {
	...
	private func setupDummyView() {
		...
		let secuondButton = UIButton(type: .custom)
		secuondButton.translatesAutoresizingMaskIntoConstraints = false
		secuondButton.setTitle("Kreis", for: .normal)
		secuondButton.addTarget(self, action: #selector(didTapSelectRegionButton), for: .touchUpInside)
		...
		let stackView = UIStackView(arrangedSubviews: [firstButton, secuondButton, thirdButton])
