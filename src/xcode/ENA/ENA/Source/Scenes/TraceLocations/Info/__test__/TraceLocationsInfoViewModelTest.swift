...
class TraceLocationsInfoViewModelTest: XCTestCase {
		...
		func testGIVEN_ViewModel_WHEN_GetNumberOfSections_THEN_CountMatchExpectations() {
			// GIVEN
			let viewModel = TraceLocationsInfoViewModel(presentDisclaimer: {})
		...
		func testGIVEN_ViewModel_WHEN_Header_THEN_ImageMatch() {
			...
			case .image(let image, _, _, _, _):
				XCTAssertEqual(image, UIImage(imageLiteralResourceName: "Illu_TraceLocations-Information"))
			default:
