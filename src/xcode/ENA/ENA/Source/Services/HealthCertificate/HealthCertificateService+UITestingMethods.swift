...
extension HealthCertificateService {
	...
	func configureForTesting() {
		...
		if LaunchArguments.healthCertificate.firstHealthCertificate.boolValue {
			registerHealthCertificate(base45: HealthCertificateMocks.firstBase45Mock, checkSignatureUpfront: shouldCheckSignatureUpfront, completion: { _ in })
			...
			if case let .success(base45) = secondDose {
				registerHealthCertificate(base45: base45, checkSignatureUpfront: shouldCheckSignatureUpfront, completion: { _ in })
			}
		...
		if LaunchArguments.healthCertificate.familyCertificates.boolValue {
			...
			if case let .success(base45) = testCert1 {
				registerHealthCertificate(base45: base45, completion: { _ in })
			}
		...
		if LaunchArguments.healthCertificate.newTestCertificateRegistered.boolValue {
			...
			if case let .success(base45) = result {
				registerHealthCertificate(base45: base45, checkSignatureUpfront: shouldCheckSignatureUpfront, markAsNew: true, completion: { _ in })
			}
		}
