...
class HealthCertificate_DeleteCertificateTests: XCTestCase {
	...
	func testGIVEN_PersonWith3Certificates_WHEN_CertificateIsDeleted_THEN_RemainingStaysAtSamePersons() throws {
		...
		let certificate2 = try HealthCertificate(
			base45:
				base45Fake(
					from: DigitalCovidCertificate.fake(
						name: Name.fake(
		...
		let certificate3 = try HealthCertificate(
			base45:
				base45Fake(
					from: DigitalCovidCertificate.fake(
						name: Name.fake(
...
//		service.registerHealthCertificate(base45: certificate3)
		...
		service.addHealthCertificate(
			certificate2,
			to: service.healthCertifiedPersons.first!
		)
		...
		let certificateToRemove = person.healthCertificates.first(where: { $0 == certificate2 }) else {
