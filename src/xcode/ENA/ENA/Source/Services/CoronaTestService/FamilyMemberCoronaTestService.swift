...
class FamilyMemberCoronaTestService: FamilyMemberCoronaTestServiceProviding {
		...
		if let testResult = mockTestResult(for: .pcr) {
			return .pcr(
				FamilyMemberPCRTest(
					...
					testResultWasShown: LaunchArguments.familyTest.pcr.positiveTestResultWasShown.boolValue,
					certificateSupportedByPointOfCare: true,
		...
		if let testResult = mockTestResult(for: .antigen) {
			return .antigen(
				FamilyMemberAntigenTest(
					...
					testResult: testResult,
					finalTestResultReceivedDate: testResult == .pending ? nil : Date(),
					testResultWasShown: LaunchArguments.familyTest.antigen.positiveTestResultWasShown.boolValue,
	...
	private func mockTestResult(for coronaTestType: CoronaTestType) -> TestResult? {
		...
		case .pcr:
			return LaunchArguments.familyTest.pcr.testResult.stringValue.flatMap { TestResult(stringValue: $0, coronaTestType: .pcr) }
		case .antigen:
			return LaunchArguments.familyTest.antigen.testResult.stringValue.flatMap { TestResult(stringValue: $0, coronaTestType: .antigen) }
		}
	}
