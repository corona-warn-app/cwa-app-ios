...
class TrustEvaluationTests: XCTestCase {
    ...
    func test_CheckServerKeyAgainstJWKSetSucccess() throws {
        ...
        let jwkSet = [
            try jsonEncoder.encode(JSONWebKey(x5x: derBase64Key, kid: keyIdentifier, alg: "", use: "")),
            try jsonEncoder.encode(JSONWebKey(x5x: "", kid: "", alg: "", use: "")),
            try jsonEncoder.encode(JSONWebKey(x5x: "", kid: "", alg: "", use: ""))
        ]
        let derBase64 = try XCTUnwrap(Data(base64Encoded: derBase64Key))
    ...
    func test_CheckServerKeyAgainstJWKSetError_CERT_PIN_NO_JWK_FOR_KID() throws {
        let jsonEncoder = JSONEncoder()
        let jwkSet = [
            try jsonEncoder.encode(JSONWebKey(x5x: derBase64Key, kid: "", alg: "", use: "")),
            try jsonEncoder.encode(JSONWebKey(x5x: "", kid: "", alg: "", use: "")),
            try jsonEncoder.encode(JSONWebKey(x5x: "", kid: "", alg: "", use: ""))
        ]
        let derBase64 = try XCTUnwrap(Data(base64Encoded: derBase64Key))
    ...
    func test_CheckServerKeyAgainstJWKSetError_CERT_PIN_MISMATCH() throws {
        ...
        let jsonEncoder = JSONEncoder()
        let jwkSet = [
            try jsonEncoder.encode(JSONWebKey(x5x: "MIIKYjCCCUq", kid: keyIdentifier, alg: "", use: "")),
            try jsonEncoder.encode(JSONWebKey(x5x: "", kid: "", alg: "", use: "")),
            try jsonEncoder.encode(JSONWebKey(x5x: "", kid: "", alg: "", use: ""))
        ]
        let derBase64 = try XCTUnwrap(Data(base64Encoded: derBase64Key))
