...
class TrustEvaluationTests: XCTestCase {
    ...
    func test_CheckServerCertificateAgainstAllowlist_Success() throws {
        ...
        let status = SecTrustCreateWithCertificates(
            [certificate] as AnyObject,
            .none,
            &optionalTrust
        )
        guard status == errSecSuccess else {
            XCTFail("Couldn't create trust")
            return
        }
        ...
        case .failure(let error):
            XCTFail("expected to pass evaluation but got error \(error.localizedDescription)")
        default:
            break
        }
    }
    
    func test_CheckServerCertificateAgainstAllowlist_FingerPrintNotFound_Then_CERT_PIN_MISMATCH() throws {
        let data = try XCTUnwrap(Data(base64Encoded: derBase64Key))
        // the following code to create the trust is taken from apple developer documentation site
        let certificate = try XCTUnwrap(SecCertificateCreateWithData(nil, data as CFData))
        var optionalTrust: SecTrust?
        ...
        switch result {
        case .failure(let error):
            XCTAssertEqual(error, .CERT_PIN_MISMATCH, "expected to fail evaluation but with error CERT_PIN_MISMATCH")
        case .success:
    ...
    func test_CheckServerCertificateAgainstAllowlist_HostNotFound_Then_CERT_PIN_HOST_MISMATCH() throws {
        ...
        switch result {
        case .failure(let error):
            XCTAssertEqual(error, .CERT_PIN_HOST_MISMATCH, "expected to fail evaluation but with error CERT_PIN_HOST_MISMATCH")
        case .success:
            XCTFail("expected to fail evaluation but with error CERT_PIN_HOST_MISMATCH")
        }
    }
