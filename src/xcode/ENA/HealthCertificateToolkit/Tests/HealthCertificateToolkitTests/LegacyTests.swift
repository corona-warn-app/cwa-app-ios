...
class LegacyTest: XCTestCase {
    ...
    func test_Extracting130CertificateFrom100Base45() {
        ...
        let certificate = DigitalGreenCertificate100.fake(
            testEntries: [
                TestEntry100.fake(dateTimeOfTestResult: "2021-05-29T22:34:17.595Z")
            ]
        )
        let base45Result = DigitalGreenCertificateFake.makeBase45Fake(from: certificate, and: CBORWebTokenHeader.fake())
        ...
        let extractResult = DigitalGreenCertificateAccess().extractDigitalGreenCertificate(from: base45)
